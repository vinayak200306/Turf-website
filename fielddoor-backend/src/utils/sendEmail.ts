import type { Booking, Slot, Sport, User, Venue } from "@prisma/client";
...
const sendMailSafe = async (to: string, subject: string, html: string) => {
  try {
...
export const sendBookingConfirmationEmail = async (
  ...
  const html = wrap(
    ...
    `
      ...
      <table style="width:100%;border-collapse:collapse;margin:20px 0;">
        <tr><td style="padding:8px 0;">Slot Fee</td><td style="padding:8px 0;text-align:right;">${formatCurrency(booking.slotFee)}</td></tr>
        <tr><td style="padding:8px 0;">GST (18%)</td><td style="padding:8px 0;text-align:right;">${formatCurrency(booking.gstAmount)}</td></tr>
        <tr><td style="padding:8px 0;">Convenience Fee</td><td style="padding:8px 0;text-align:right;">${formatCurrency(booking.convenienceFee)}</td></tr>
        <tr><td style="padding:8px 0;border-top:1px solid rgba(255,255,255,0.12);"><strong>Total Paid</strong></td><td style="padding:8px 0;border-top:1px solid rgba(255,255,255,0.12);text-align:right;"><strong>${formatCurrency(booking.totalAmount)}</strong></td></tr>
      </table>
  ...
  await sendMailSafe(
    user.email ?? "",
    `Booking Confirmed – ${booking.bookingRef} | Field Door`,
    html
  );
};

export const sendPaymentFailureEmail = async (user: User, booking: Booking, venue: Venue) => {
  const html = wrap(
    ...
    `
      ...
      <p><a href="${env.APP_URL}/bookings/${booking.id}" style="display:inline-block;padding:12px 20px;border-radius:999px;background:#7B61FF;color:#fff;text-decoration:none;">Try Again</a></p>
...
export const sendCancellationEmail = async (
  ...
  const refundCopy =
    refundAmount > 0
      ? `A refund of ${formatCurrency(refundAmount)} will be credited to your original payment method within 5–7 business days.`
...
export const sendRefundProcessedEmail = async (
  ...
  const html = wrap(
    ...
    `
      <p>Booking Reference: <strong>${booking.bookingRef}</strong></p>
      <p>Refund Amount: <strong>${formatCurrency(refundAmount)}</strong></p>
  ...
  await sendMailSafe(
    user.email ?? "",
    `Refund Processed – ${booking.bookingRef} | Field Door`,
    html
  );
};
